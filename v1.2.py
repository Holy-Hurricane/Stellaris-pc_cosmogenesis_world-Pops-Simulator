...
try:
    v2=eval(input("请输入全局维护修正，默认为-5%（繁荣/普适传统），单位为“%”："))
except:
    v2=-5
    print("输入无效，取用默认值-5%")
finally:
...
gne=eval(input("请输入初始帝国总人口数量："))
if gne-m<0:
    print("玩儿呢，帝国人口比突触凝练机少？")
    em+=1
gen=eval(input("请输入增长需求倍率："))
if gen<0:
    em=input("负倍率你确定不会出问题？")
print("数据输入完毕，现在开始模拟！")
try:
...
fl.writelines("月数,人口,单岗研究,总研究,单岗超然逻辑,总超然逻辑,单岗能量币维护,总岗能量币维护,人口净化进度,产生人口净化进度,人口增长进度,产生人口增长进度,帝国人口\n")
while u2>0:
    w=w+1
    g=g0*m    #共振器产出修正
    b0=i+3*a+j+st+v1    #产出固定修正之和
    b=b0+g    #总修正
    ...
    d=d1//100    #当月净化人口数
    gth1=gh1+gh2
    gg=100+gen*gne    #人口增长需求
    gth=gth1//gg    #当月人口增长数
    d=int(d)
    m=int(m)    
    gth=int(gth)    #对这三个值取整，打印出来好看些
    ls=[str(w-1),str(m),str(tk1),str(tk2),str(lg1),str(lg2),str(mt1),str(mt2),str(u1),str(u2),str(gh1),str(gh2),str(gne)]
    ...
    gh1=gth1%gg    #下个月的已完成人口增长进度
    m=m-d+gth    #下一个月的人口
    gne=gne-d+gth    #下一个月的帝国人口
    if w>=T:
        break
